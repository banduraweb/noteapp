...
class UserService {
  static async registration(name, email, password) {
    const { error } = Validation.registrationValidation({ name, email, password });
    ...
    try {
      ...
      if (isExists) {
        return SystemErrorService.error('An user with this email already existed', errorTypes.Validation);
      }
  ...
  static async login(email, password) {
    ...
    try {
      const isExists = await User.findOne({ email });
      if (!isExists) {
        return SystemErrorService.error('User was not found', errorTypes.NotFound);
      }
      ...
      if (!isMatchedPassword) {
        return SystemErrorService.error('Not correct password', errorTypes.Validation);
      }
